...
```
확장 단계   컬럼과 테이블을 추가만 한다. 기존 것을 지우거나 이름을 바꾸지 않는다
축소 단계   새 코드가 배포되고 안정된 것을 확인한 "다음 작업일 이후" 에 지운다
한 머지에 확장과 축소를 함께 담지 않는다. 담는 순간 그 배포는 되돌릴 수 없는 배포가 된다
```

- 마이그레이션이 든 PR 은 템플릿의 세 번째 검증 칸에 확장인지 축소인지, 한 머지에 둘이 섞이지 않았는지 적는다
...
- (단지와 표시 그룹) 조합의 **25.21%는 최근 12개월 거래가 없다** (서울 25구에서 36개월간 등장한 조합 15,975개 중, 2026-09-05 측정). 판정 배지를 붙이지 않는다
- 표본을 볼 때 `numOfRows`를 크게 잡고 분포를 본다. **첫 페이지는 표본이 아니다**

## 보고 형식

판단 근거가 되는 사실 주장에는 등급을 붙인다. 코드와 설명문에는 붙이지 않는다.
[T] 도구 결과(커맨드+출력) [F] 파일(경로:줄) [K] 미검증 일반지식
[I] 추론(무엇을 결합했는지) [G] 제안이나 추측
